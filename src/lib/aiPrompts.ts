...
// ============ 每日思考题：评分（题目从头部基金题库抽取） ============

// 评分 system prompt
const SYSTEM_PROMPT_DAILY_EVALUATOR = `你是一位参与过睿远、淡水泉、高毅、景林等头部主观基金投研面试的资深合伙人，正在评估候选人今日的深度思考题作答。
...
2. 引用候选人答案中的具体表述作为论据。
3. **与参考答案对比**：明确指出候选人遗漏或不及参考答案的维度，但承认候选人独到的视角。
4. 修改建议给出参考思路的关键维度，但不代写完整答案。
5. 全部用 HTML 格式（<h4>/<p>/<ul>/<li>/<strong>），不要 markdown。`;
...
export function buildDailyAnswerEvaluationMessages(input: {
  ...
  referenceKeywords: string[];
  referenceAnswer: string;  // 参考答案（HTML）
  answer: string;
}): ChatMessage[] {
  const userPrompt = `【题目来源】${input.sourceSummary}
【思考维度】${input.dimension}
【题目标题】${input.title}
...

{
  ...
  "feedback": "<HTML 格式的点评，200-400 字，先肯定亮点再指出问题，与参考答案对比>",
  "improvements": "<HTML 格式的修改建议，200-400 字，给出参考思路的关键维度与具体可补强的方向>"
}
